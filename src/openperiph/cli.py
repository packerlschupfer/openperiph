#!/usr/bin/env python3
"""
openperiph — Generic CLI for gaming peripherals.
...
from openperiph import find_device, __version__
...
def print_global(device: PeripheralDevice):
...
def print_profile(device: PeripheralDevice, profile: int):
    caps = device.capabilities
    print(f"\n── Profile {profile} {'─'*47}")
    try:
...
def print_all(device: PeripheralDevice):
    print_global(device)
...
def build_parser(caps=None):
    ...
    p = argparse.ArgumentParser(
        prog='openperiph',
        description='OpenPeriph Linux configuration tool',
    ...
    try:
        if args.import_file:
            ...
            if data.get('format') != 'openperiph-profile':
                sys.exit("Error: not a valid openperiph profile file")
