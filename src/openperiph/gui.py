#!/usr/bin/env python3
"""
openperiph-gui — GTK4/libadwaita settings GUI + system-tray applet
                    for gaming peripherals.

Run directly:
    python3 -m openperiph.gui
...
APP_ID = 'io.github.packerlschupfer.OpenPeriph'
...
ICON_NAME = 'openperiph'


def _logo_image(size: int) -> Gtk.Image:
    """The project's own logo (data/openperiph.svg, by @Scout339), at *size* px.

    Installed packages put it in the hicolor theme, where looking it up by
    name works. Running straight from a source tree there is no theme entry,
    and the name resolves to the broken-image placeholder -- so fall back to
    loading the file out of the checkout, and to a stock icon if even that is
    missing (an installed-but-icon-less setup, or a stripped-down package).
    """
    display = Gdk.Display.get_default()
    if display is not None and Gtk.IconTheme.get_for_display(display).has_icon(ICON_NAME):
        img = Gtk.Image.new_from_icon_name(ICON_NAME)
        img.set_pixel_size(size)
        return img

    # gui.py -> openperiph/ -> src/ -> repo root, then data/
    root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    svg = os.path.join(root, 'data', f'{ICON_NAME}.svg')
    if os.path.exists(svg):
        try:
            # set_pixel_size only applies to named icons, so a paintable
            # needs an explicit size request instead.
            img = Gtk.Image.new_from_paintable(Gdk.Texture.new_from_filename(svg))
            img.set_size_request(size, size)
            return img
        except Exception:
            pass

    img = Gtk.Image.new_from_icon_name('input-mouse')
    img.set_pixel_size(size)
    return img


def _rgba(r: float, g: float, b: float, a: float = 1.0) -> Gdk.RGBA:
    """Build a Gdk.RGBA by assignment rather than through its constructor.

    Passing field values to Gdk.RGBA() goes through gi.types.Boxed.__init__(),
    which is deprecated and silently ignores every argument -- so the
    constructor form does not merely warn, it will start yielding black once
    PyGObject drops it.
    """
    rgba = Gdk.RGBA()
...
class OpenPeriphApp(Adw.Application):
    ...
    _BATTERY_STATE_PATH = os.path.expanduser('~/.cache/openperiph/battery.json')
    ...
    def __init__(self):
        ...
        self._device = None  # PeripheralDevice instance

    def _find_or_create_device(self) -> PeripheralDevice | None:
    ...
    def _build_tray(self, win: 'MainWindow', device: PeripheralDevice):
        caps = device.capabilities
        sni = _StatusNotifierItem('openperiph', 'input-mouse', caps.name)
...
class MainWindow(Adw.ApplicationWindow):
    def __init__(self, device: PeripheralDevice | None = None, **kwargs):
        super().__init__(**kwargs)
        self._device = device
        self._caps = device.capabilities if device else None
        self.set_title(self._caps.name if self._caps else 'OpenPeriph')
    ...
    def _build_ui(self):
        ...
        # The Power page holds only the two wireless power-management
        # sliders, both hasattr-gated on the driver (see
        # _build_power_page). On a wired device it has nothing to show, so
        # the row is left out entirely rather than opening a blank pane.
        self._has_power_page = (hasattr(self._device, 'set_power_saving_timeout')
                                or hasattr(self._device, 'set_low_power_threshold'))
        ...
        sidebar_page = Adw.NavigationPage.new(sidebar_toolbar, 'OpenPeriph')
        ...
        if self._has_power_page:
            ...
            # _build_power_page() would have set these; _do_apply() and
            # _populate_global() both test them for None.
    ...
    def _build_home_page(self):
        ...
        # Hidden until a version actually arrives: drivers that don't
        # override get_firmware_version() return 'unknown', and a
        # permanently blank row reads as a failed read rather than a device
        # that simply doesn't report one. The CLI omits the line for the
        # same reason. _refresh_firmware_version() reveals it.
    ...
    def _build_performance_page(self):
        ...
        for i in range(1, caps.max_dpi_stages + 1):
            ...
            if caps.has_stage_colors:
                ...
                color_btn.set_rgba(_rgba(1.0, 1.0, 1.0))
        ...
        if _is_hyprland_detected():
            ...
            hypr_apply.set_tooltip_text(
                ...
                'openperiph-input.conf" to your hyprland.conf once so it '
    ...
    # one-time `source = ~/.config/hypr/openperiph-input.conf` line in
    # the user's own hyprland.conf, same pattern many other Hyprland addon
    # tools use for the same reason.
    _HYPR_CONF_PATH = os.path.expanduser('~/.config/hypr/openperiph-input.conf')
    ...
    _AUTOSTART_PATH = os.path.expanduser('~/.config/autostart/openperiph.desktop')
    _AUTOSTART_CONTENT = """\
[Desktop Entry]
Name=OpenPeriph
Comment=OpenPeriph system-tray applet
Exec=openperiph-gui
Icon=input-mouse
Type=Application
...
"""
    ...
    def _start_home_updates(self):
        ...
        (OpenPeriphApp._hidraw_listener) - deliberately not shared, so
        ...
        driven entirely by OpenPeriphApp._set_battery_label() (see its
        ...
        def _read():
            try:
                ...
                if fw != 'unknown':
                    ...
                    GLib.idle_add(self._firmware_row.set_visible, True)
            except Exception:
                pass
        threading.Thread(target=_read, daemon=True).start()
    ...
    def _do_remap_button(self, btn_id, btn_type, a1, a2):
        ...
        try:
            ...
            GLib.idle_add(self._btn_rows[btn_id].set_subtitle,
                          self._device.describe_button(t, ra1, ra2))
    ...
    def _on_import_finish(self, dialog, result):
        ...
        def _do():
            try:
                ...
                return
            if data.get('format') != 'openperiph-profile':
    ...
    def _populate_profile(self, lod, brightness, led, breath, dpi_info, colors, buttons):
        ...
        if self._color_buttons and colors:
            for i, btn in enumerate(self._color_buttons):
                ...
                if rgb is not None:
                    r, g, b = rgb
                    btn.set_rgba(_rgba(r / 255, g / 255, b / 255))
...
class RemapButtonDialog(Adw.Window):
    ...
    def __init__(self, btn_label: str, current_spec: str, on_applied,
                 device=None, **kwargs):
        super().__init__(**kwargs, title=f'Remap {btn_label}',
                         default_width=360, default_height=-1)
        self.set_modal(True)
        self._on_applied = on_applied
        # The dropdown vocabulary below is still hid.py's Pulsar/Sonix set;
        # only the parse goes through the driver. A device with its own
        # encoding overrides parse_button_function() and advertises its
        # vocabulary via button_function_names() -- wiring that into these
        # categories is the remaining step for a foreign-vendor driver.
        self._device = device
    ...
    def _on_set_clicked(self, _btn):
        try:
            ...
            parse = (self._device.parse_button_function if self._device
                     else parse_button_function)
            t, a1, a2 = parse(spec)
