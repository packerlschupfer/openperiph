...
from openperiph.hid import BTN_TYPE_MOUSE, BTN_TYPE_DPI
...
class PulsarX2A(PeripheralDevice):
    ...
    # Ack polling, see _poll_ack(). 8 x 20ms bounds a stuck command
    # at ~160ms -- slow enough to ride out the gap the device needs,
    # fast enough that a genuinely dead command still fails promptly.
    _ACK_POLLS = 8
    _ACK_DELAY = 0.02
    ...
    def _poll_ack(self, accept: tuple) -> bytes:
        """GET_REPORT until byte[0] is one of *accept*, or polls run out.

        The device does not always have its reply ready on the first
        GET_REPORT after a SET_REPORT: it answers 0x00 (nothing yet) and
        needs a moment. Back-to-back commands hit this constantly -- the
        LED writes are the worst, since setting brightness plus one colour
        per DPI stage fires ~7 commands with no gap, and roughly half of
        them used to fail. Reads are affected too, so a failed write could
        be followed by a failed read of the same field.

        Poll the *same* report rather than resending the command:
        re-issuing SET_REPORT restarts the exchange and races the reply
        that was already on its way (feinmann8k.py's _query_ctrl()
        docstring covers the same trap on that device's RF link, where the
        latency is far larger).
        """
    ...
    def _write_led_block(self, profile: int, effect_val: int, speed: int) -> None:
        """Write the effect+speed register as one unit.

        Register 0x03/0x04/0x0F holds both: byte[8] is the effect, byte[11]
        the breath speed (that is where get_led_effect and get_breath_speed
        read them from). Writing it with only one of the two filled in zeroes
        the other, so both setters read the current block first and put back
        the field they are not changing.

        Bytes 9-10 are left at zero, matching the Windows captures this
        protocol came from -- their meaning is unknown, so echoing back
        whatever the device reports there would be a guess, not a fix.
        """
    ...
    def set_breath_speed(self, speed: int, profile: int) -> None:
        ...
        # Was hardcoding 0x02 ('breath') into the effect byte, so setting the
        # speed switched the LED into breathing mode behind the user's back.
        rsp = self._read(0x03, 0x04, 0x0F, profile, [0x01])
